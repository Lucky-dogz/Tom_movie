  ...
  <div class="cart" style="width: 100%">
    ...
    <div v-loading="loading">
      <el-card v-for="(item, index) in cartList" :key="index" class="box-card">
        <div>
          ...
          <div style="float: left;">
            ...
            <div class="item-film-time">
              开场时间 :
              <span style="color: red"
                >{{ item.arrangement.date }} -
                {{ item.arrangement.startTime }}</span
              >
            </div>
          </div>
          <div
            style="float: right;color: #f34d41;letter-spacing: 2px; line-height: 100px;padding-right: 5px"
          >
            ￥{{ item.cart.price }}
            <el-button
      ...
      <div
        ...
        style="height: 60px;background: #91949c"
      >
        <div>
          <div
            style="float: left;line-height: 60px;padding: 0 20px;font-size: 14px"
          >
            <el-checkbox @change="handleCheckAll" v-model="checkAll"
              >全 选</el-checkbox
            >
          </div>
          <el-button @click="submitCart" type="text" class="sub-btn"
            >全部结算</el-button
          >
