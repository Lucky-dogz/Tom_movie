<template>
  <div>
    ...
    <div
      style="font-weight: bolder;font-size: 25px;padding-top: 10px;padding-bottom: 10px;border-top:1px solid #dedede"
    >
      剧情简介
    </div>
    <div
      style="line-height: 30px;text-indent:2.5em;padding-top: 10px;padding-bottom: 20px"
    >
      {{ film.introduction }}
    </div>
    <div
    ...
    <div style="padding-right: 100px">
      <div class="item" v-if="evaluateList.length === 0">暂无评论</div>
      <div class="item" v-for="item in evaluateList">
        <img
          alt=""
          style="float: left;width: 55px;height: 55px;border-radius: 50%"
          :src="item.user.avatar"
        />
        <div class="header-name">
          <div
            v-if="item.user.nickname === null"
        ...
        <i style="float: right;padding-right: 18px;color: #91949c"
          >Time : {{ item.filmEvaluate.createAt }}</i
        >
      </div>
    </div>
  </div>
